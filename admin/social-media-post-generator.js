...
const DEFAULT_TEXTAREA_TEXT = `#This is a heading
...
#This is another heading bleep bleep blorp blorp
...
for (const [elementName, htmlId] of Object.entries(DOMelementsThatUpdateCanvasOnInput)) {
	...
	if (htmlId === "image-width-input") {
		...
		callback = (ev) => {
			for (const canvasElement of document.querySelectorAll("canvas")) {
				canvasElement.width = ev.target.value;
			}
		...
		callback = (ev) => {
			for (const canvasElement of document.querySelectorAll("canvas")) {
				canvasElement.height = ev.target.value;
			}
			updateCanvas(ctx, fontFamilyName, DOMelementsThatUpdateCanvasOnInput);
		};
...
document.getElementById("download-button").addEventListener("click", () => {
	for (const [index, canvasElement] of Array.from(document.querySelectorAll("canvas")).entries()) {
		canvasElement.toBlob((blob) => {
			const a = document.createElement("a");
			a.href = window.URL.createObjectURL(blob);
			// remember, indexes start from 0
			a.download = `Tiger Tank social media post - image ${index + 1}`;
			a.click();
		});
	}
});
